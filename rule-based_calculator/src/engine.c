...
calc_error_t calc_init(calc_ctx_t *ctx) {
    ...
    memcpy(ctx->operator_table, ops, sizeof(ops));
...
calc_error_t calc_add(calc_ctx_t *ctx, calc_num_t a, calc_num_t b,
                      calc_num_t *out) {
...
calc_error_t calc_subtract(calc_ctx_t *ctx, calc_num_t a, calc_num_t b,
                            calc_num_t *out) {
    if (ctx == NULL || out == NULL) {
        return CALC_ERR_NULL_POINTER;
    }
...
calc_error_t calc_multiply(calc_ctx_t *ctx, calc_num_t a, calc_num_t b,
                            calc_num_t *out) {
    if (ctx == NULL || out == NULL) {
        return CALC_ERR_NULL_POINTER;
    }
...
calc_error_t calc_divide(calc_ctx_t *ctx, calc_num_t a, calc_num_t b,
                          calc_num_t *out) {
    if (ctx == NULL || out == NULL) {
        return CALC_ERR_NULL_POINTER;
    }
...
Operator *calc_get_operator(calc_ctx_t *ctx, const char *symbol) {
    if (ctx == NULL || symbol == NULL) {
        return NULL;                    /* was: return calc_strerror(...) */
    }
    ...
    return NULL;                        /* was: return calc_strerror(CALC_ERR_OK) */
}

calc_error_t calc_evaluate(calc_ctx_t *ctx, const Token *tokens,
                            size_t token_count, calc_num_t *result) {
    ...
    for (size_t i = 0; i < token_count; i++) {
        if (tokens[i].token_type == TOKEN_END) {
            ...
            if (stack_top >= MAX_TOKENS) return CALC_ERR_OVERFLOW;
